...
d.lihc.clinical = data.frame(matrix(NA, nrow = length(v.file.names), ncol = length(v.clinical.name)), stringsAsFactors = F)
colnames(d.lihc.clinical) = v.clinical.name
...
for (i.file in 1:length(v.file.names)) {
  ...
  # add to data.frame
  d.lihc.clinical[i.file,] = v.xml.value[v.clinical.name]
}
